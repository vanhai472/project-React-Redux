...
import {
  ...
  buffVideoV2Failed,
  buffVideoV2Success,
  getTypeBufVideoSuccess,
  getTypeBufVideoFailed
} from "../actions/user";
...
function* watchFetchBuffVideo({ payload }) {
  const { params } = payload;
  yield put(showLoading());
  try {
    const resp = yield call(getTypeBuffVideo, params);
    const { data, status } = resp;
    if (status === STATUS_CODE.SUCCESS) {
      yield put(getTypeBufVideoSuccess(data));
    } else {
      yield put(getTypeBufVideoFailed(data));
    }
  } catch (error) {
    yield put(getTypeBufVideoFailed(error));
  } finally {
    yield delay(1000);
    yield put(hideLoading());
  }
...
function* userSaga() {
  ...
  yield takeLatest(types.GET_TYPE_BUFF_VIDEO, watchFetchBuffVideo);
