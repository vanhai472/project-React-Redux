...
class BuffViewVideo extends React.Component {
  ...
  componentDidMount() {
    ...
    const { getTypeBufVideo } = userAction;
    if (getTypeBufVideo) {
      const params = {
        token
      };
      getTypeBufVideo(params);
    }
  }
      ...
      (err, values) => {
        if (!err) {
          ...
          if (
            ...
            values.service_id = form.getFieldValue("service_id");
            buffVideoV2(values);
          }
        }
      }
                ...
                >
                  ...
                  >
                    ...
                    paramUrl?.seeding === "buff-seeding-view-video" ? (
                      ...
                      >
                        {getFieldDecorator("service_id", {
                          ...
                          <Select size="large" className="input-blue">
                            {dataTypeBuffVideo &&                                                     
                            dataTypeBuffVideo.length > 0 ? (                                                          
                              dataTypeBuffVideo.map(el => {
                                return (
                              ...
                              <Option value="">Tăng view loại thường</Option>
                            )}
                          </Select>
                        )}
                      </Form.Item>
                    ...
                    <Button
                      ...
                      loading={isLoadingBuffVideo || isLoadingBuffSub}
                    >
                      Tạo tiến trình
                    </Button>
                  </Form>
...
const mapStateToProps = state => ({
  ...
  dataTypeBuffVideo: state.user.dataTypeBuffVideo,
  isLoadingBuffVideo: state.user.isLoadingBuffVideo,
  isLoadingBuffSub: state.user.isLoadingBuffSub
});
